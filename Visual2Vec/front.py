...
import glob
import pickle
import tdt76.tdt76_main as your_code


main_path = '/home/havikbot/'      # Please provide the main path here
...
if __name__ == "__main__":
    ...
    havikbot = your_code.main(main_path)

    # First calls here to make sure we have generated a list of all IDS and their labels stored in a pickle
    train_labels = generate_dict_from_directory(pickle_file=main_path+'train/pickle/combined.pickle', directory=main_path+'/train/txt/')
    ...
    test_labels = generate_dict_from_directory(pickle_file=main_path+'test/pickle/combined.pickle', directory=main_path+'/test/txt/')
    ...
    results = havikbot.test(queries=queries, location=main_path+'test/pics/')     # location = test_folder
