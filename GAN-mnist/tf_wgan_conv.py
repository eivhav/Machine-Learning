...
def create_2Dimage(image_vector, dim):
    picture = []
    for i in range(dim):
        picture.append(list(image_vector[dim*i:dim*(i+1)]))
...
def discriminator(x, weights):
    ...
    out = tf.nn.sigmoid(tf.add(tf.matmul(pool1, weights['w2']), weights['b2']))
...
fake_hat = discriminator(create_2Dimage(sample, 28), weights_D)
real_hat = discriminator(create_2Dimage(X, 28), weights_D)
